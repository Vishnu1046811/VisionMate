...
class DiaryLogger(context: Context) : CoroutineScope {
    ...
    fun saveDescriptionToFile(content: String) {
    ...
    fun captureAndSummarize(image: Bitmap, onComplete: (summary: String?) -> Unit) {
        Log.e("TAG", "captureAndSummarize: ", )
        launch (Dispatchers.IO){
            try {
                val summary = getDescriptionFromScene(image) ?: throw Exception("Failed")
                withContext(Dispatchers.Main) {
                    onComplete(summary)
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    onComplete("Unable to summarize at this moment ")
                }
    ...
    companion object {
        lateinit var INSTANCE: DiaryLogger

        fun init(context: Context) {
            INSTANCE = DiaryLogger(context)
        }
